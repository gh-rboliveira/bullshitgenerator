$(document).ready(function() {
    fetch("https://us-central1-intranet-262014.cloudfunctions.net/hitCounter")
    ...
    $("#overlay-opener").on("click", function() {
        $("#overlay").show();
    });
...
function buzzword() {
    ...
    if (!adverb.classList.contains('locked')) {
        adverbs = randomarray(adverbs);
        adverb.innerHTML = (adverbs[adverbs.length - 1]).toUpperCase();
    }
    if (!verb.classList.contains('locked')) {
        verbs = randomarray(verbs);
        verb.innerHTML = (verbs[verbs.length - 1]).toUpperCase();
    }
    if (!adjective.classList.contains('locked')) {
        adjectives = randomarray(adjectives);
        adjective.innerHTML = (adjectives[adjectives.length - 1]).toUpperCase();
    }
    if (!noun.classList.contains('locked')) {
        nouns = randomarray(nouns);
        noun.innerHTML = (nouns[nouns.length - 1]).toUpperCase();
    }
}